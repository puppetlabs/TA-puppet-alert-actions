...
def run_puppet_plan(alert, helper):
 ...
 # Check if the user is configured with an RBAC token or Password:
  ...
  try:
    jobid = pie.bolt.reqplan(plan_name,auth_token,puppet_environment,bolt_url,parameters=plan_parameters)
  ...
  try:
    jobresults = pie.bolt.getplanresult(jobid, auth_token, bolt_url, timeout=plan_timeout)
  except Exception as e:
  ...
  rmessage['joburl'] = 'https://{}/#/plan_jobs/{}'.format(pe_console,jobid)
...
# alert['global']['puppet_bolt_server'] = "puppet_enterprise_console"
